...
function setupBuffers() {
    ...
    pointBuffer = gl.createBuffer(); // 추가
...
function setupMouseEvents() {
    ...
    function handleMouseUp() {
        if (isDrawing && tempEndPoint) {
            ...
            lines.push([...startPoint, ...tempEndPoint]); 
            radius = getDistance(lines[0][0].toFixed(2), lines[0][1].toFixed(2), lines[0][2].toFixed(2), lines[0][3].toFixed(2))
            if (lines.length == 1) {
                radius = getDistance(...lines[0]);
                const cx = lines[0][0];
                const cy = lines[0][1];
                circleVertices = generateCircleVertices(cx, cy, radius);
            ...
            if (lines.length == 2) {
                ...
                // 교차점 계산해서 배열에 저장
                intersectionPoints = computeCircleSegmentIntersections(lines[0], radius, lines[1]);
            
                // 텍스트 출력
                if (intersectionPoints.length > 0) {
                    let str = `Intersection Points: ${intersectionPoints.length}`;
                    intersectionPoints.forEach((p, i) => {
                        str += ` Point ${i + 1}: (${p[0].toFixed(2)}, ${p[1].toFixed(2)})`;
                    });
                    updateText(textOverlay3, str);
                } else {
    ...
    function computeCircleSegmentIntersections(circleLine, r, segLine) {
        const cx = circleLine[0], cy = circleLine[1];
        const x1 = segLine[0], y1 = segLine[1];
        const x2 = segLine[2], y2 = segLine[3];
...
function generateCircleVertices(cx, cy, radius, segments = 100) {
    const vertices = [];
...
function render() {
    ...
    if (circleVertices.length > 0) {
        ...
        shader.setVec4("u_color", [1.0, 1.0, 0.0, 1.0]); // 노란 원
        gl.bindVertexArray(vao);
